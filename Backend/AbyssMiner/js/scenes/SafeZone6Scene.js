...
class SafeZone6Scene extends MainGameScene {
    ...
    create() {
        ...
        // (SZ6 空画布) GridSystem/世界边界从出生点 (col -38, row 16) 四向扩: 左 20 / 右 200 / 上 100 / 下 100 格
        const originX = -58 * G;                 // 出生点左 20 格 (col -58)
        const originY = -84 * G;                 // 出生点上 100 格 (row -84)
        const totalW  = (162 - (-58) + 1) * G;   // 到 col 162 (右 200 格) = 221 格宽
        const totalH  = (116 - (-84) + 1) * G;   // 到 row 116 (下 100 格) = 201 格高
        ...
        // (SZ6 空画布) 不建 SZ3 地形; 出生点 (col -38, row 16) 正下方放一排原皮 cavetile 小平台
        { const _G = 32; for (let c = -43; c <= -33; c++) new CavetileWall(this, c * _G + _G / 2, 17 * _G + _G / 2, _G, _G); }
        ...
        // (SZ6 空画布) 已删除 SZ3 全部 CrystalNpc (双子/chief/Amber/Citrine/Toy/Watch/Solo/Encourage); update 里的引用都带守卫, 安全

        // (SZ6 空画布) 不启动 SZ3 开局过场, 直接解锁玩家
